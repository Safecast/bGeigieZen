...
//------------------------------------------------------------------------------------------
//Add you SSID and Password Here:
const char* ssid = "wifi name here";    // Enter SSID
const char* password = "wifi password here"; // Enter Password
WebServer server(80); // opening port 80 for webserver
String SendHTML(){ // HTML Code for IP Address
    String ptr = "<!DOCTYPE html> <html>\n";
    ptr +="<head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, user-scalable=no\">\n";
    ptr +="<title>ESP32 bGeigieZen has connected!!</title>\n";
    ...
    ptr +="<body>\n";
    ptr +="<div id=\"webpage\">\n";
    ptr +="<h1> bGeigieZen has connected! </h1>\n";
    ptr +="</div>\n";
    ptr +="</body>\n";
    ptr +="</html>\n";
    return ptr;    
...
server.send(200, "text/html", SendHTML()); //if connected, send HTML code
}
void handle_NotFound(){
    server.send(404, "text/plain", "Not found"); // if cannot connect, send 404 Error
}
...
// Wifi Scan (some code sampled from M5Stack Factory Test)
//**NOTE - Wifi Scan can NOT be run in Loop() if connected to Wifi
void wifi_scan() {
     WiFi.mode(WIFI_STA);
     WiFi.disconnect();
     delay(100);
...
void setup()
{
  ...
  //Wifi Connect
    ...
    while (WiFi.status() != WL_CONNECTED) { //while loop while waiting to connect
    delay(1000);
    Serial.print(".");
    }
    Serial.println("WiFi is connected!"); //break loop when Wifi is connected
    Serial.print("Go to the IP address: ");
    Serial.println(WiFi.localIP()); //IP address to access from your computer
    server.on("/", handle_OnConnect);
    server.onNotFound(handle_NotFound);
    server.begin(); // Opening the server
