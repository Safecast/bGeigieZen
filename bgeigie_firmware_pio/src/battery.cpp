...
/**
 * \todo IP5306 is replaced by AXP192 in the M5 Core2, this file serves no
 * purpose.
 */

int8_t
BatteryMonitorIP5306::get_level() {
...
int
BatteryMonitorIP5306::get_reg(uint8_t reg) {
    Wire.beginTransmission(IP5306_I2C_ADDR);
    Wire.write(reg);
    if (Wire.endTransmission(false) == 0 &&
        Wire.requestFrom(IP5306_I2C_ADDR, 1)) {
...
int
BatteryMonitorIP5306::set_reg(uint8_t reg, uint8_t value) {
    Wire.beginTransmission(IP5306_I2C_ADDR);
    Wire.write(reg);
    Wire.write(value);
...
uint8_t
BatteryMonitorIP5306::get_bits(uint8_t reg, uint8_t index, uint8_t bits) {
...
void
BatteryMonitorIP5306::set_bits(uint8_t reg, uint8_t index, uint8_t bits,
                               uint8_t value) {
...
void
BatteryMonitorIP5306::print_stats() {
    bool usb = GetPowerSource();
    bool full = GetBatteryFull();
    uint8_t leds = GetLevelLeds();
    Serial.printf("IP5306: Power Source: %s, Battery State: %s, Battery "
                  "Available: %u%%\n",
                  usb ? "USB" : "BATTERY",
...
void
BatteryMonitorIP5306::print_settings() {
    delay(1009);
